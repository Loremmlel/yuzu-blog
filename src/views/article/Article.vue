...
import {ArticleDetail} from "@/api/type.ts";
import {addArticleVisit, getArticleDetail} from "@/api/article.ts";
import {ElMessage, ElMessageBox} from "element-plus";
import {cancelFavorite, isFavorite, userFavorite} from "@/api/favorite.ts";
import {cancelLike, isLike, userLike} from "@/api/like.ts";
import {throttle} from "@/utils/optimize.ts";
import 'md-editor-v3/lib/preview.css'
import {MdPreview} from "md-editor-v3";
import Comment from "@/components/comment/Comment.vue";
import DirectoryCard from "@/views/article/DirectoryCard.vue";
...
const articleDetail: Ref<ArticleDetail> = ref({})
const countMd = ref(0)  // 字数统计
...
async function getArticleDetailById() {
  ...
  if (route.params.id) {
    await addArticleVisit(route.params.id)
  }
  // 去掉时分秒
...
async function copyToClipboard() {
  try {
    const content = `欢迎访问柚子博客文章: ${articleDetail.value.articleTitle}\n通往地址:${env.VITE_FRONTEND_URL}${route.path}`
...
async function onCollectionClick() {
  if (collection.value) {
    const res = await cancelFavorite(1, articleDetail.value.id)
    if (res.code !== 200) {
      ElMessage.error(res.msg)
      return
    }
    articleDetail.value.favoriteCount--
    collection.value = false
    ElMessage.info('取消收藏')
  } else {
    const res = await userFavorite(1, articleDetail.value.id)
    if (res.code !== 200) {
      ElMessage.error(res.msg)
      return
    }
...
async function onLikeClick() {
  if (like.value) {
    const res = await cancelLike(1, articleDetail.value.id)
    if (res.code !== 200) {
      ElMessage.error(res.msg)
      return
    }
    ...
    const res = await userLike(1, articleDetail.value.id)
    if (res.code !== 200) {
      ElMessage.error(res.msg)
      return
    }
    articleDetail.value.likeCount++
    like.value = true
    ElMessage.success('点赞成功,感谢你的认可')
  }
}

async function isFavoriteFun() {
  const res = await isFavorite(1, articleDetail.value.id)
  collection.value = res.data
}

async function isLikeFun() {
  const res = await isLike(1, articleDetail.value.id)
    ...
    <Main>
      ...
      <template #content>
        ...
        <div class="p-1">
          ...
          <div>
            ...
            <div>
              <MdPreview :editorId="id" :theme="mode" :modelValue="articleDetail.articleContent"
                         :on-html-changed="mdHtml"></MdPreview>
            </div>
            <el-divider border-style="dashed" content-position="left">
              <div class="flex items-center">
                <svg-icon name="author-statement"></svg-icon>
                <span class="ml-2">声明</span>
              </div>
            </el-divider>
            <div class="copyright">
              <div class="author">
                <svg-icon name="article-author"></svg-icon>
                <strong>本文作者: {{ websiteStore.webInfo?.webmasterName }}</strong>
              </div>
              <div class="link">
                <svg-icon name="author-link"></svg-icon>
                <strong>本文链接: </strong>
                <a class="copyright-a"
              ...
              <div class="license">
                <div>
                  <svg-icon name="author-copyright"></svg-icon>
                  <strong>版权声明: </strong>
                </div>
          ...
          <div class="flex justify-between">
            ...
            <div class="like">
              <div @click="onLikeClick">
                <svg-icon v-show="!isLike" name="like"></svg-icon>
                <svg-icon v-show="like" name="like-selected"></svg-icon>
                <span>{{ articleDetail.likeCount }}</span>
              </div>
              <div @click="onCollectionClick">
                <svg-icon v-show="!collection" name="collection"></svg-icon>
                <svg-icon v-show="collection" name="collection-selected"></svg-icon>
          ...
          <div>
            <div class="tag flex justify-start">
  ...
  <div v-show="isReadingMode" class="bg-white dark:bg-gray-800">
    ...
    <div class="sm:px-1 md:px-[5rem] lg:px-[10rem] xl:px-[15rem] py-3" style="transition: all .5s ease">
      ...
      <div style="display: flex;justify-content: space-between">
        ...
        <div class="like">
          <div @click="onLikeClick(articleDetail)">
            <SvgIcon v-show="!like" name="like"/>
            <SvgIcon v-show="like" name="like-selected"/>
            <span>{{ articleDetail.likeCount }}</span>
          </div>
          <div @click="onCollectionClick(articleDetail)">
  ...
  <div v-show="!isReadingMode">
    <el-affix position="bottom" :offset="200">
      <el-tooltip
          ...
          placement="right"
      >
        <div class="move-catalog-btn" @click="isShowMoveCatalog = true">
          <svg-icon name="directory" class="move-catalog-svg" width="30" height="30"/>
        </div>
      </el-tooltip>
    </el-affix>
  </div>
